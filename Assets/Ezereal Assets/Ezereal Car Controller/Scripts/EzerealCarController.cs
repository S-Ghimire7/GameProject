   ...
   void ApplyAcceleration()
    {
        ...
        float baseTorque = horsePower * ratio * finalDrive * traction;

        float torque = baseTorque * throttleInput;

        if (currentGear == ManualGears.Gear1)
        {
            if (speedKmh < 5f)
            {
                torque += baseTorque * 0.35f;
            }

            float ramp = Mathf.Clamp01(speedKmh / 20f);
            torque *= Mathf.Lerp(0.4f, 1f, ramp);
        }
